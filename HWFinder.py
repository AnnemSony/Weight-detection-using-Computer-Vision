...
if __name__ == "__main__":
    ...
    # Loading models with map_location for CPU if CUDA is not available
    ...
    # Using GPU if available, otherwise using CPU
    if torch.cuda.is_available():
        model_w = model_w.cuda(args.gpu)
    else:
        model_w = model_w
    ...
    out_name = args.image.split(os.path.sep)[-1].replace(fformat, '.mask.png')
    out_name_j = args.image.split(os.path.sep)[-1].replace(fformat, '.joint.png')
    out_name_sk = args.image.split(os.path.sep)[-1].replace(fformat, '.skeleton.png')
    
    with open(os.path.join(output_dir, args.image.split(os.path.sep)[-1].replace(fformat, '.info.txt')), 'w') as out_file:
        out_file.write("Image: " + args.image)
        out_file.write("\nHeight: {:.1f} cm\nWeight: {:.1f} kg".format(100*h_p.item(), 100*w_p.item()))
        
    cv2.imwrite(os.path.join(output_dir, out_name), (255*mask_out).astype('uint8'))
    plt.imsave(os.path.join(output_dir, out_name_j), joint_out, cmap='jet')
    plt.imsave(os.path.join(output_dir, out_name_sk), img_sk)
